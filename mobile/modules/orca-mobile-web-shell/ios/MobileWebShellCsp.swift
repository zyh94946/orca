enum MobileWebShellCsp {
  ...
  static let header = [
    ...
    // upload the host names; and whatever an artifact names inside the sealed HTML preview frame,
    // which inherits this policy because a `srcdoc` frame has no URL of its own.
    //
    ...
    // plain source field, and the avatar is skipped under `Platform.OS !== 'web'` by a card that
    // pins the skip in a test. All three are anticipated surfaces, and C7.10 is where they become
    // real ones.
    //
